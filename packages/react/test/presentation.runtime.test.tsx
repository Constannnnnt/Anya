...
import type { FileStorage, PresentationPlan } from '@anya-ui/core';
...
function RuntimeHarness(props?: {
  onInteractionExecuted?: React.ComponentProps<typeof AdaptiveRuntimeRenderer>['onInteractionExecuted'];
}) {
  const { registerTool, commitPresentationPlan } = useAnyaUI();
  ...
  }, [commitPresentationPlan, registerTool]);

  return <AdaptiveRuntimeRenderer onInteractionExecuted={props?.onInteractionExecuted} />;
}

describe('presentation runtime integration', () => {
  it('executes native binding actions through AdaptiveRuntimeRenderer', async () => {
    render(
  ...
  it('hydrates session state and persistent profile into presentation context', async () => {
    ...
    await waitFor(() => {
      expect(result.current.presentationState.context.persistentProfile).toContain('Persisted Profile');
    });
    await waitFor(() => {
      expect(result.current.presentationState.currentSpec?.components[0].id).toBe('persist-text');
    });
    await waitFor(() => {
    ...
    expect(result.current.presentationState.currentSpec?.components[0].id).toBe('persist-text');
    expect(result.current.presentationState.context.sessionHistory?.[0].semanticDescription)
