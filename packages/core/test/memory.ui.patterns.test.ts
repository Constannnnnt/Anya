...
import { buildPatternCandidate } from '../src/memory/ui/patterns';
    ...
    const events: UiMemoryEvent[] = [
      ...
      makeEvent({
        ...
        payloadJson: JSON.stringify({
          record: { action: 'custom', componentName: 'Button', elementId: 'btn-1' },
        }),
      }),
      makeEvent({
  ...
  it('uses episode assessment for outcome and task class', () => {
    const events: UiMemoryEvent[] = [
      makeEvent({
        type: 'interaction.recorded',
        payloadJson: JSON.stringify({
          record: { action: 'submit', componentName: 'Form', elementId: 'f1' },
        }),
      }),
      makeEvent({
        type: 'binding.executed',
  ...
  it('returns null when sequence signal is too short', () => {
    const events: UiMemoryEvent[] = [
      makeEvent({
        type: 'interaction.recorded',
        payloadJson: JSON.stringify({
          record: { action: 'expand', componentName: 'Card', elementId: 'c1' },
        }),
      }),
    ];
  ...
  it('dedupes consecutive repeated steps', () => {
    const events: UiMemoryEvent[] = [
      makeEvent({
        type: 'interaction.recorded',
        payloadJson: JSON.stringify({
          record: { action: 'custom', componentName: 'Button', elementId: 'b1' },
        }),
      }),
      makeEvent({
        type: 'interaction.recorded',
