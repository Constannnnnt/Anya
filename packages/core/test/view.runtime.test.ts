...
import type { ActionBinding, ToolDefinition } from '../src/views/types';
import { createViewEngine } from '../src/views/engine';
import { planView } from '../src/views/updatePlanner';
import { buildViewFromState } from '../src/views/builder';
import { resolveBindingValue, ToolRuntime } from '../src/views/actions';
...
describe('view runtime (v0 scenarios)', () => {
  ...
  it('applies optimistic value_change updates to mapped component props and shared data without breaking $data bindings', async () => {
    ...
    expect(current?.components[0].props.value).toEqual({ $data: { nodeId: 'params', path: 'hidden' } });
    expect(current?.components[1].props.content).toBe(24);
    expect(current?.components[2].props.datasets).toEqual([{ label: 'Value', data: [24] }]);
