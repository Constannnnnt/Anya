...
import type { FileStorage } from '../src/storage/interface';
...
describe('ContextMemoryManager', () => {
  ...
  it('starts a new task scope by clearing volatile session UI context', () => {
    ...
    memory.saveCurrentSpec({
      layout: 'stack',
      components: [{ id: 'person-card', type: 'Card', props: { title: 'Peng Shi' } }],
    });
    memory.recordInteraction({
      timestamp: 1,
      elementId: 'person-card',
      componentName: 'Card',
      action: 'custom',
  ...
  it('records interactions and respects max length', () => {
    ...
    expect(recent[0].elementId).toBe('e2');
    expect(recent[1].elementId).toBe('e3');
  });
  ...
  it('hydrates context/spec/interactions from persisted snapshot', async () => {
    ...
    memoryA.saveCurrentSpec({
      layout: 'stack',
      components: [{ id: 'restored', type: 'Heading', props: { text: 'Restored' } }],
    });
    ...
    expect(memoryB.getCurrentSpec()?.components[0].id).toBe('restored');
