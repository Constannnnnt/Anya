 ...
 */
import type { UIComponentSpec, UIInteractionRecord, UIRenderSpec } from '../types';
...
function collectSpecComponents(spec: UIRenderSpec | null): Map<string, UIComponentSpec> {
  const components = new Map<string, UIComponentSpec>();
  if (!spec) return components;

  const stack = [...spec.components];
  while (stack.length > 0) {
    const next = stack.pop()!;
    components.set(next.id!, next);
...
export function createViewEngine(opts?: {
  ...
  return {
    ...
    setContext(patch) {
      ...
      let nextSpec: UIRenderSpec | null = state.currentSpec;
      try {
        transact((current) => ({
          nextState: {
            ...current,
    ...
    async executeInteraction(interaction) {
      return enqueueInteraction(async () => {
        ...
        let workingSpec: UIRenderSpec = applyOptimisticUpdate(startState.currentSpec, interaction);
        ...
        for (const binding of matchingBindings) {
          ...
          if (action.type === 'tool_call') {
            ...
            if (policy.risk === 'risky') {
              ...
              workingSpec = setViewNodeProp(workingSpec, binding.componentId, 'busy', true);
              commitPreview(workingSpec);
            }
          }
          ...
          if (resolvedRisk === 'risky') {
            nextSpec = setViewNodeProp(nextSpec, binding.componentId, 'busy', false);
          }
