 ...
 */
import type { InteractionTrigger, UIComponentSpec, UIRenderSpec } from '../types';
import type {
  ActionBinding,
  ActionCommand,
  StateNode,
...
export const DEFAULT_VIEW_COMPONENT_SLOTS: ViewComponentSlots = {
  heading: 'Heading',
  card: 'Card',
  image: 'Image',
  list: 'List',
...
function resolveViewComponentSlots(
  overrides?: Partial<ViewComponentSlots>,
): ViewComponentSlots {
  return {
...
function buildStateNodeComponent(
  node: StateNode,
  componentSlots: ViewComponentSlots,
): UIComponentSpec {
...
function buildToolComponentsAndBindings(
  ...
  componentSlots: ViewComponentSlots,
): { components: UIComponentSpec[]; bindings: ActionBinding[] } {
  if (tools.length === 0) {
...
function chooseLayout(options?: BuildViewFromStateOptions): UIRenderSpec['layout'] {
...
function collectComponentIds(components: UIComponentSpec[]): Set<string> {
  ...
  const walk = (nodes: UIComponentSpec[]) => {
...
function inferBoundDataPath(component: UIComponentSpec, nodeId: string): string | undefined {
...
function buildWorkflowContextComponents(
  ...
  if (definition?.components?.length) {
    summaryParts.push(`Components: ${definition.components.join(', ')}`);
  }
...
export function buildViewFromState(
  ...
  const componentSlots = resolveViewComponentSlots(options?.projectionComponents);
  ...
  const allComponents: UIComponentSpec[] = [...workflowComponents];
...
export function extractActionBindings(spec: UIRenderSpec): ViewPlan {
  const bindings: ActionBinding[] = [];
  const componentIds = collectComponentIds(spec.components);
  ...
  const walk = (components: UIComponentSpec[]) => {
    for (const component of components) {
      ...
      for (const rawTarget of component.bindTo ?? []) {
        ...
        if (componentIds.has(target.targetId)) {
          continue;
        }
