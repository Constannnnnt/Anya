  ...
  sections.push(
    opts?.preamble ??
      [
        ...
        '- Timeline, List, and Section components have built-in spacing — do not add custom `gap` overrides below their defaults.',
        '- When in doubt, use MORE spacing. A spacious UI is always better than a cramped one.',
        '',
        '# INTERACTION RULES',
        '- Attach an `interactions` array to ANY component to make it interactive. Triggers: onClick, onDoubleClick, onMouseEnter, onMouseLeave.',
        ...
        '- EVENT BROADCASTING: If an interaction controls other components, link them using `targetIds` (e.g., targetIds: ["video-1"]) and `targetAction` (e.g. "play").',
        '- LOCAL DATA SYNC: To synchronize the visual state of a component (like a Slider value) to another component (like a Text label), use `bindTo: ["target-id"]`.',
        ...
        '1. Rethink the layout: Switch between FlexRow/FlexCol/Grid/split to optimize the presentation.',
...
export function buildResponseFormatBlock(format: 'yaml' | 'json'): string {
  if (format === 'json') {
    return [
      ...
      '  "components": [',
      '    { "id": "my-id", "type": "ComponentName", "props": { ... }, "children": [...] }',
      '  ]',
      '}',
      '```',
    ].join('\n');
  ...
  return [
    ...
    '        description: "What this click semantically means in this context"',
    '        tool_call:',
    '          name: "rotate-image"',
    '          parameters:',
    '            angle: 90',
    ...
    'Use `children` to nest components inside containers.',
    '',
