  ...
  sections.push(
    opts?.preamble ??
      [
        ...
        '- EVENT BROADCASTING: If an interaction controls other components semantically, link them using `targetIds` (e.g., targetIds: ["video-1"]) and `targetAction` (e.g. "play").',
        '- Do NOT use `targetIds` + `targetAction: setValue/setChecked/setContent` as a state-mutation shortcut. For charts or preset controls, drive components from shared `$data` state or use an explicit local patch/data-update binding.',
