/**
 * @anya-ui/core — ContextMemoryManager
 *
...
export class ContextMemoryManager {
  ...
  private currentSpec: UIRenderSpec | null = null;
  ...
  saveCurrentSpec(spec: UIRenderSpec): void {
    this.currentSpec = spec;
    this.trackElements(spec.components);
  ...
  getCurrentSpec(): UIRenderSpec | null {
  ...
  recordInteraction(record: UIInteractionRecord): void {
    ...
    const hist = this.elementHistories.get(record.elementId);
    if (hist) {
      hist.actions.push({
    ...
    if (recent.length > 0) {
      ...
      for (const r of recent) {
        const desc = r.semanticDescription
          ?? `${r.componentName}.${r.action}(${r.propName ?? ''})`;
        lines.push(`- ${desc}`);
      }
      lines.push('');
    }
  ...
  private specToMarkdown(spec: UIRenderSpec, indent = ''): string {
    const lines: string[] = [];
  ...
  private hydrateFromSnapshot(snapshot: MemorySnapshot): void {
    ...
    if (this.currentSpec && this.elementHistories.size === 0) {
      this.trackElements(this.currentSpec.components);
    }
  }
