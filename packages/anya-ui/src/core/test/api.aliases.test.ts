...
import { InMemoryBehaviorStore } from '../experimental';
import { createViewEngine as createViewEngineNamespace } from '../views';
...
describe('public API cleanup', () => {
  it('exports only the canonical runtime and view entrypoints from the main barrel', () => {
    ...
    expect('ViewSpec' in api).toBe(false);
    expect('ViewNode' in api).toBe(false);
