...
import type { ViewSpec, ViewMetadata } from '../types';
...
function countComponents(nodes: ViewSpec['nodes']): number {
  let count = 0;
  const stack = [...nodes];
...
export function createViewChangeDraft(input: CreateViewChangeDraftInput): ViewChangeDraft {
  ...
  return {
    ...
    impact: {
      baselineComponentCount: countComponents(input.currentSpec.nodes),
      proposedComponentCount: countComponents(input.proposedSpec.nodes),
