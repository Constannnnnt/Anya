...
function projectSingleEvent(
  ...
  if (event.type === 'interaction.measured') {
    ...
    const nodeType = asString(payload?.nodeType) ?? 'unknown';
    const action = asString(payload?.action) ?? 'custom';
    const componentFamily = asString(measurement?.componentFamily) ?? inferComponentFamily(nodeType);
  ...
  if (event.type === 'binding.executed') {
    ...
    const nodeType = asString(interaction?.nodeType) ?? 'unknown';
    const action = asString(interaction?.action) ?? 'custom';
    return {
      ...createBaseSignal(event, view),
      contextArchetype: deriveContextArchetype(view, inferComponentFamily(nodeType), inferActionFamily(action)),
      componentFamily: inferComponentFamily(nodeType),
      actionFamily: inferActionFamily(action),
      modality: inferModalityFromInteraction(nodeType, asString(interaction?.trigger)),
  ...
  if (event.type === 'tool.finished' || event.type === 'tool.failed') {
    ...
    return {
      ...createBaseSignal(event, view),
      contextArchetype: deriveContextArchetype(view, inferComponentFamily(nodeType), 'tool'),
      componentFamily: inferComponentFamily(nodeType),
      actionFamily: 'tool',
      modality: inferModalityFromInteraction(nodeType, asString(interaction?.trigger)),
