...
import { AnyaProvider, useAnyaContext } from '../Provider';
import {
  createRuntimeEvent,
  getLogger,
  setLogger,
...
describe('AnyaProvider & useAnyaContext', () => {
    it('throws an error if useAnyaContext is used outside Provider', () => {
        ...
        expect(() => {
            renderHook(() => useAnyaContext()); }).toThrowError('useAnyaContext must be used within an <AnyaProvider>');
    ...
    it('provides the framework context when wrapped in AnyaProvider', () => {
        ...
        expect(result.current.nodeMap).toBeDefined();
        expect(result.current.nodeMap.has('Heading')).toBe(true); });
    ...
    it('emits telemetry and failure budget signals through provider hooks', async () => {
        ...
        act(() => {
            result.current.runtime.dispatch(createRuntimeEvent('spec.decode_failed', { error: 'e1' } as any));
            result.current.runtime.dispatch(createRuntimeEvent('spec.decoded', {
                spec: { layout: 'stack', nodes: [] } as any, } as any));
            result.current.runtime.dispatch(createRuntimeEvent('spec.decode_failed', { error: 'e2' } as any)); });
        ...
        act(() => {
            result.current.runtime.dispatch(createRuntimeEvent('ui.presented', {
                view: {
                    ...
                    actionFamilies: ['activate'], } as any, }, { source: 'system' }));
            result.current.runtime.dispatch(createRuntimeEvent('interaction.measured', {
                ...
                action: 'submit',
                measurement: {
                    modality: 'pointer' as any,
                    componentFamily: 'action',
                    actionFamily: 'activate',
                    choiceSetSize: 6, } as any, }, { source: 'user' }));
            result.current.runtime.dispatch(createRuntimeEvent('session.status_set', { status: 'thinking' as AgentState }, { source: 'system' }));
            result.current.runtime.dispatch(createRuntimeEvent('session.status_set', { status: 'idle' as AgentState }, { source: 'system' })); });
    ...
    it('warns when mount-only provider props change after initialization', async () => {
        ...
        try {
            ...
            expect(warningMessages.some((message) => message.includes("'uiMemory' is mount-only"))).toBe(true); } finally {
            restore(); } });
    ...
    it('does not warn when mount-only props are recreated with equivalent values', async () => {
        ...
        try {
            ...
            expect(warn).not.toHaveBeenCalled(); } finally {
            restore(); } }); });
