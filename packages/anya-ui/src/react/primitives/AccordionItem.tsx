...
import { bindDrag, useSyncedState, type PrimitiveBehaviorProps, type PrimitiveRenderProps } from './shared';
...
export const AccordionItem = defineComponent({
    ...
    render: ({ id, props, children, onInteraction }: PrimitiveRenderProps<AccordionItemProps>) => {
        const [isExpanded, setIsExpanded] = useSyncedState(props.defaultExpanded, false);
        ...
        return (
            <div
                id={id}
                className={`anya-accordion-item ${isExpanded ? 'active' : ''} ${props.draggable ? 'anya-draggable' : ''} ${props.className || ''}`}
                style={props.style}
            ...
            >
                <button
                    type="button"
                    className="anya-accordion-trigger"
