/**
 * ../react — AdaptiveRenderer
 *
 * The core rendering component that takes a ViewSpec from your
 * agent and renders the corresponding React nodes.
 *
...
import { getLogger, normalizeStyleProp, resolveBindingValue, type InteractionModality, type ViewSpec, type ViewNode, type InteractionEvent, type InteractionSpec } from '../core';
import { useAnyaContext } from './Provider';
import type { AnyaNode, AnyaRenderProps } from './defineComponent';
...
function RenderComponent({
    ...
    const handleInteraction: AnyaRenderProps['onInteraction'] = (action, detail) => {
        ...
        const plugin = nodeMap?.get(spec.type);
...
function deriveDynamicChoiceSetSize(
    ...
    const choiceChildren = (parentSpec.children ?? []).filter((child: ViewNode) =>
