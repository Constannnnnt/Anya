...
import { registerNodeRun, removeNodeRegistrationRun } from './useAnyaUI/componentActions';
import {
...
export function useAnyaUI(): UseAnyaUI {
  ...
  const removeNodeRegistration = useCallback((name: string) => {
    removeNodeRegistrationRun(ctx, name);
  }, [ctx]);
  ...
  const registerNode = useCallback(
    (node: AnyaNode) =>
      registerNodeRun(ctx, node, removeNodeRegistration),
    [ctx, removeNodeRegistration]
  );

  const unregisterNode = useCallback(
    (name: string) => removeNodeRegistration(name),
